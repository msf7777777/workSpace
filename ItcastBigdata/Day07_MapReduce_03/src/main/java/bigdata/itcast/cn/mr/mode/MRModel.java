package bigdata.itcast.cn.mr.mode;
...
public class MRModel extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setJarByClass(MRModel.class);//指定可以运行的类型
        //todo:2-配置这个Job
        //input
//        job.setInputFormatClass(TextInputFormat.class);//设置输入的类的类型，默认就是TextInputFormat
        Path inputPath = new Path(args[0]);//用程序的第一个参数做为第一个输入路径
...
//        job.setOutputFormatClass(TextOutputFormat.class);//设置输出的类，默认我诶TextOutputFormat
        Path outputPath = new Path(args[1]);//用程序的第三个参数作为输出
